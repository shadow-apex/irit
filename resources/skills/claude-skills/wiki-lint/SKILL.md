...
### Step 5b - Check for over-length index entries

For each entry in `index.md`, measure the one-line description (the text between `]]:` and the trailing `` `path` ``). If it exceeds 150 characters, flag as an **over-length index entry** (soft warning). index.md is read in full by every operational skill on each run, so long descriptions inflate that per-run cost. Suggested fix: tighten to 150 chars max.
...
### Step 6e - Check log.md size

Count the `## [` entries in `log.md`. log.md is prepend-only and read in full by crystallize and lint on every run, so an oversized log is a standing context cost. If there are **more than 150** entries, flag as a **log oversized** finding with the entry count and the threshold (150). lint never modifies log.md; the fix is to run wiki-crystallize, which offers to archive the oldest entries and keep the 50 most recent.
...
- Conceptual flags: N
- Log size: N entries (flagged if over 150)
...
## Over-length Index Entries
[index entry, current description length, suggested: tighten to 150 chars max]
...
## Log Size
[log.md entry count; flagged if over 150 - run wiki-crystallize to archive the oldest entries, keeping the 50 most recent]
```
...
- Over-length index entries -> shorten the description to 150 chars max
...
- Log oversized -> run wiki-crystallize; it offers to archive the oldest log entries, keeping the 50 most recent
