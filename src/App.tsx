...
export default function App() {
  ...
  useEffect(() => {
    function onKey(event: KeyboardEvent) {
      ...
      // Esc closes the second-brain galaxy overlay — it has no in-panel close
      // control of its own (unlike the drawing panel, which handles Esc
      // internally), and the fullscreen overlay can sit over the toggle
      // button that would otherwise close it.
      if (key === "escape" && secondBrainActive) {
        event.preventDefault();
        toggleSecondBrain();
        return;
      }
  ...
  }, [sidecarRunning, hasBridge, secondBrainActive]);
  ...
  // Live Teleprompter (Alt+T): "content" là phụ đề/dịch (3 dòng gần nhất);
  // copilotHistory/copilotStatus (khi Nhắc bài bật) là dữ liệu có cấu trúc
  // riêng cho bảng gợi ý cuộn được — xem HudShell.
  ...
  const [teleprompterState, setTeleprompterState] = useState<{
  ...
  useEffect(() => {
    if (!hasBridge || hudMessage?.title !== "Live Teleprompter") return;
    window.iris.getTeleprompterState().then(setTeleprompterState).catch(() => {});
  }, [hasBridge, hudMessage]);

  const handleToggleTranslate = useCallback(async (lang: string) => {
    if (!hasBridge) return;
    await window.iris.toggleTranslate(lang);
    const next = await window.iris.getTeleprompterState();
    setTeleprompterState(next);
  }, [hasBridge]);

  const handleToggleInterviewCopilot = useCallback(async () => {
    if (!hasBridge) return;
    await window.iris.toggleInterviewCopilot();
    const next = await window.iris.getTeleprompterState();
    setTeleprompterState(next);
  }, [hasBridge]);
  ...
  useEffect(() => {
    if (!hasBridge) return;
    return window.iris.onHudMessage(
      (msg: { title: string; content: string; copilotHistory?: TeleprompterCopilotEntry[]; copilotStatus?: string }) => {
        const now = Date.now();
        ...
        lastHudMsgRef.current = { title: msg.title, content: msg.content, time: now };
        setHudMessage(msg);
      }
    );
  }, [hasBridge]);
  ...
  useEffect(() => {
    ...
    // FIX: trước đây MỌI hudMessage (kể cả báo cáo/tóm tắt dài như
    // "Tóm tắt cuộc họp" hay phụ đề Live Teleprompter) đều bị đóng cứng
    // sau đúng 3000ms, khiến báo cáo dài biến mất trước khi đọc kịp.
    // Giờ thời gian hiển thị được tính theo độ dài nội dung (ước lượng
    // tốc độ đọc ~200 từ/phút + 2s dự phòng), tối thiểu 3s (giữ nguyên
    // hành vi cũ cho các thông báo trạng thái ngắn như "Đang ghi âm...")
    // và tối đa 60s cho báo cáo dài. Người dùng vẫn có thể bấm nút đóng
    // (X) để tắt sớm bất cứ lúc nào.
    //
    // Live Teleprompter là NGOẠI LỆ: nó tự quản lý vòng đời hiển thị của
    // chính nó (main process liên tục đẩy cập nhật khi có transcript/dịch/
    // gợi ý mới, và chủ động gửi "Đã tắt tính năng..." rồi null khi người
    // dùng tắt Alt+T — xem toggleLiveTranscriber trong main.mjs). Nếu vẫn
    // để timer chung này chạy, chỉ cần cuộc trò chuyện im lặng ~10-15s
    // (rất bình thường trong 1 cuộc họp) là toàn bộ HUD — bao gồm cả bảng
    // lịch sử câu hỏi/gợi ý của Nhắc bài — sẽ tự đóng, đúng thứ người dùng
    // yêu cầu KHÔNG được xảy ra ("không bị mất khi sếp hỏi câu khác").
    if (hudMessage.title === "Live Teleprompter") return;
    const text = `${hudMessage.title ?? ""} ${hudMessage.content ?? ""}`;
    const wordCount = text.trim().split(/\s+/).filter(Boolean).length;
    const readingTimeMs = (wordCount / 200) * 60000;
    const duration = Math.min(Math.max(readingTimeMs + 2000, 3000), 60000);
    ...
    <>
      {uiMode === "hud" ? (
        <HudShell
          ...
          onToggleInterviewCopilot={handleToggleInterviewCopilot}
