...
// --- Ported from myiris (mrqhocungdungai-vn/myiris) ---
...
// Ported from myiris's pipeline-probes.mjs `pipelineAvailable` flag, in
// simplified form: irit has no separate OpenSpec pipeline-install gate on
// plain-Claude use, so this just tracks whether the `claude` CLI itself was
// last seen reachable. Starts optimistic (true) so the canvas MCP / second
// brain aren't gated off before the first checkClaudeStatus() probe runs.
let claudePipelineAvailable = true;
...
// prompt-review-gate (ported from myiris): one slot, same "raised -> settled
// exactly once" invariant as PendingQuestion above, so a park can never be
// left dangling or resolved twice. Holds no execution slot (no run, no
// run_id) — zero Claude tokens spent until approved.
const PendingReview = {
...
// prompt-review-gate (ported from myiris, design.md D-review): read fresh on
// every submitClaudeTask call (never cached) so a toggle mid-session takes
// effect on the very next submit.
...
// prompt-review-gate (ported from myiris): event type stays `task_review`
// for renderer/IPC — carries the brief text so the deck banner can prefill
// its editable textarea (this is the one exception to "never log the brief"
// below, since it is a deliberate UI push, not a console log).
...
function dispatchClaudeRun({ task, urgency = "normal", agent, workstream }) {
  const cleanTask = String(task).trim();
...
// UI (deck ReviewBanner "Cancel") and voice (respond_to_task_review) both
// call this; whichever resolves first wins.
