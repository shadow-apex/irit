...
Names the execution behavior the delegation model already relies on but no spec covers: the one-at-a-time Claude execution slot shared by PO turns, DEV runs, and plain Claude tasks, its queueing/cancellation lifecycle, and the `claude_task_update` event stream it produces.
