...
Lets the PO, DEV, and STUDY pipeline roles each run on an independently chosen Claude model, selectable per workstream via UI or voice, resolved fresh at run start with no automatic fallback, and traceable per run. (TBD: expand with broader rationale if needed.)
